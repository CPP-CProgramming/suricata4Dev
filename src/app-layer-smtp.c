...
static int SMTPProcessRequest3(SMTPState *state, Flow *f,
                              AppLayerParserState *pstate)
{
    SCEnter();
    ...
    if(state->current_command == SMTP_COMMAND_DATA && begin)
    {
            //printf("CHECK HERE\n");
			if (!(state->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE))
            {
            ...
            if (state->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE)
            {
                if(state->eml_started != 1)
                {
                    ...
                    if (FileOpenFile2(state->files_ts, (uint8_t *) filename, 5,
                            (uint8_t *) state->current_line, state->current_line_len) == NULL) 
                    {
                        SCLogDebug("FileOpenFile() failed");
                        printf("FileOpenFile() failed\n");
                        SCReturnInt(-1);
                    }
					else
						printf("FileOpenFile %d\n", count++);

                                                
                    FileAppendData2(state->files_ts, (uint8_t *) "\n", 1);                                           
                }
                else
                {
                   // SCLogDebug("Appending file...%u bytes", state->current_line_len);
                   // printf("Appending file...%u bytes(%d)\n", state->current_line_len, count);
                                      /* 0 is ok, -2 is not stored, -1 is error */
                    if(state->files_ts)
                    {
                                        
                        ret = FileAppendData2(state->files_ts, (uint8_t *) state->current_line, state->current_line_len);
                        if (ret < 0) 
                        {
                            //SCLogDebug("FileAppendData() failed: %d", ret);
                            printf("FileAppendData() failed: %d\n", ret);
                            ret = -1;
                            //SCReturnInt(-1);
                        }
                        else
            ...
            if (state->current_line_len == 1 && state->current_line[0] == '.')
            {
                ...
                SCLogDebug("Closing file...%u bytes", 0);
                //printf("Closing2 file...%u bytes\n", 0);
                        //if (files && files->tail && files->tail->state == FILE_STATE_OPENED) 
                        //{

				printf("FileCloseFile %d\n", ccount++);
				
                ret = FileCloseFile2(state->files_ts, (uint8_t *) NULL, 0);
				if (ccount == 200)
					printf("FileCloseFile()200\n");
				//printf("FileCloseFile %d\n", ccount++);
                if (ret != 0) 
                {
                    SCLogDebug("FileCloseFile() failed: %d", ret);
                    printf("FileCloseFile() failed: %d\n", ret);
                    ret = -1;
                }
                        //} else {
                        //    SCLogDebug("File already closed");
