...
//New metadata format
static void LogFilestoreLogCreateMetaFile2(const Packet *p, const File *ff, char *base_filename, int ipver) {
    if (!FileWriteMeta())
        return;
    ...
    if (fp != NULL) {
        ...
        if(p->flow->alproto == ALPROTO_HTTP)
        {
         ...
         if(fm)
         {
            shortname_len = ff->name_len - ((uint8_t*)pos - ff->name);
            if(shortname_len < 0)
               shortname_len = 0;
        ...
        fprintf(fp, "%s:%" PRIu16 " > %s:%" PRIu16 " bytes(%" PRIu64 ") file(%s) name(\"", srcip, sp, dstip, dp, ff->size, base_filename);
...
static const char *ProtoToString(unsigned int proto)
{
        ...
        File *ff, const uint8_t *data, uint32_t data_len, uint8_t flags, uint8_t dir)
{
    ...
    if (flags & OUTPUT_FILEDATA_FLAG_OPEN) {
        ...
        LogFilestoreLogCreateMetaFile2(p, ff, base_filename, ipver);
