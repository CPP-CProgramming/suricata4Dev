...
static int FileAppendFileData2(FileContainer *ffc, FileData *ffd);
static void FileDataFree(FileData *);
static int FileAppendFileDataFilePtr2(File *ff, FileData *ffd);
static int FileCloseFilePtr2(File *ff, const uint8_t *data,
        uint32_t data_len);
#ifdef HAVE_NSS
...
int FileAppendData2(FileContainer *ffc, const uint8_t *data, uint32_t data_len)
{
    SCEnter();
    ...
    //printf("file size is now %"PRIu64"\n", ffc->tail->size);
    
    SCLogDebug("appending %"PRIu32" bytes", data_len);
    //printf("FileAppendData2: appending %"PRIu32" bytes\n", data_len);

    FileData *ffd = FileDataAlloc(data, data_len);
	//printf("FileAppendData2: ffd=%p\n", ffd);
    if (ffd == NULL) {
    ...
    /* append the data */
    if (FileAppendFileData2(ffc, ffd) < 0) {
        ffc->tail->state = FILE_STATE_ERROR;
        FileDataFree(ffd);
        SCReturnInt(-1);
    }
...
File *FileOpenFile2(FileContainer *ffc, const uint8_t *name, uint16_t name_len,
        const uint8_t *data, uint32_t data_len)
{
    SCEnter();
	//printf("FileOpenFile2\n");
    //PrintRawDataFp(stdout, name, name_len);
    ...
    if (data != NULL) 
	{
        ...
        /* append the data */
        if (FileAppendFileData2(ffc, ffd) < 0) {
			printf("FileOpenFile2: Check here\n");
            ff->state = FILE_STATE_ERROR;
            FileDataFree(ffd);
            SCReturnPtr(NULL, "File");
        }
...
static int FileAppendFileData2(FileContainer *ffc, FileData *ffd)
{
    ...
    if (FileAppendFileDataFilePtr2(ffc->tail, ffd) == -1)
    {
        SCReturnInt(-1);
    }
...
static int FileAppendFileDataFilePtr2(File *ff, FileData *ffd)
{
	...
	//printf("FileAppendFileDataFilePtr2-------------\n");
	//printf("ff=%p, ffd=%p, ff->chunks_tail=%p\n", ff, ffd, ff->chunks_tail);
	//printf("---------------------------------------\n");
    if (ff->chunks_tail == NULL) {
		...
		//printf("148: FileAppendFileDataFilePtr2\n");
    ...
    if (data != NULL) {
        //PrintRawDataFp(stdout, data, data_len);

         {
            ...
            /* append the data */
            if (FileAppendFileDataFilePtr2(ff, ffd) < 0)
			{
				printf(" file state error\n");
                ff->state = FILE_STATE_ERROR;
                FileDataFree(ffd);
        ...
        ff->state = FILE_STATE_CLOSED;
        SCLogDebug("flowfile state transitioned to FILE_STATE_CLOSED");
		//printf("flowfile state transitioned to FILE_STATE_CLOSED\n");
...
int FileCloseFile2(FileContainer *ffc, const uint8_t *data,
        uint32_t data_len)
{
    SCEnter();
    ...
    if (FileCloseFilePtr2(ffc->tail, data, data_len) == -1) {
        SCReturnInt(-1);
    }
